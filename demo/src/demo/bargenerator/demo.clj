...
(comment
  ...
  (let [sched3 (m/eduction (take 2) scheduler)
...
;(def data [1 2 3 :x 4 5 6 :X])
;(take-while #(not (keyword? %))  data)
;(take-until keyword? data)


(defn time-buffered2 [scheduler flow]
  (m/ap
   (let [[_ restartable] (m/?> (m/group-by {} scheduler))
         [_ restartable2] (m/?> (m/group-by {} flow))
         ]
     (m/? (->> (m/ap (m/amb= 
                      (m/?> restartable) 
                      (m/? (m/reduce conj (m/eduction (take 1) restartable2)))
                      ;(m/?> restartable2)
                      ))
...
((->> ;bar-processor
  ...
  (time-buffered2 scheduler quote-feed)
  (m/eduction (take 2))
